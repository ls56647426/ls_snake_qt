#include "cmsnake.h"
#include "ui_cmsnake.h"

CMSnake::CMSnake(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CMSnake)
{
    ui->setupUi(this);

	setWindowTitle("经典贪吃蛇");
...
void CMSnake::initGame()
{
	...
	//蛇身初始化
	snake = new Snake(mapLength, mapSize);
...
//绘制游戏界面
//绘制游戏界面
void CMSnake::paintEvent(QPaintEvent *event)
{
	if(endGamestatus == -2) return;
	...
	pen.setWidth(0);
	painter.setPen(pen);
	for (int i = 2*mapSize; i <= (mapLength - 2)*mapSize; i += mapSize)
		for (int j = 2*mapSize; j <= (mapLength - 2)*mapSize; j += mapSize)
	...
	brush.setStyle(Qt::SolidPattern);
	painter.setBrush(brush);
	painter.drawEllipse(it->x, it->y, mapSize, mapSize);
	it++;
	...
	//绘制食物
	painter.drawImage(QRect(snake->food->x, snake->food->y, mapSize, mapSize), jinbi);

	QWidget::paintEvent(event);
}

//游戏结束
void CMSnake::endGame()
{
	//停止背景音乐，并播放死亡音效
	bgPlayer->stop();
	timer->stop();
	...
	{	//游戏结束的情况，-2:未开始; -1:暂停; 0:正常; 1:撞到墙; 2:咬到自己; 3:主动退出游戏;
	case 1:
		swPlayer->play();
		msgBox.setText("对不起，您撞到墙了，结束游戏！");
	...
	case 2:
		swPlayer->play();
		msgBox.setText("对不起，您咬到自己了，结束游戏！");
		msgBox.setIcon(QMessageBox::Icon::Critical);
		msgBox.exec();
		swPlayer->stop();
		break;
...
//按键按下事件
void CMSnake::keyReleaseEvent(QKeyEvent *event)
{
	if(endGamestatus == -2) return;
...
void CMSnake::on_soundOnOffCheckBox_stateChanged(int arg1)
{
	switch(arg1)
	{
